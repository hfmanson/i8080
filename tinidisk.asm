...
;
; Backported Tiny BASIC to bare bones i8080 without CP/M
; Stefan Tramm, November 2015
;
; used source is located at:
;  http://www.nicholson.com/rhn/files/tinybasic.tar.Z
;
; find details about this BASIC at:
...
;*
       ORG  0
RSTART DI             ;*** START/RSTART OR RST 0 ***
       LXI  SP,STACK  ;INIT STACK
INIPAT JMP  INIT      ;THIS IS PATCHED AFTER 1ST RUN
       DB   'L'
;*
       XTHL           ;*** TSTC OR RST 1 ***
...
GOTO   RST  3         ;*** GOTO EXPR ***
       ...
       JMP  RUNTSL    ;GO DO IT
;*
DSAVE  CALL ENDCHK    ;*** SAVE(CR) ***
       LXI  D,TXTBGN  ;HL POINTER
       LHLD TXTUNF    ;DE ENDMARKER
       XCHG
DS1    MOV  A,L       ;UNUSED MEM REACHED?
       CMP  E
       JNZ  DS2
       MOV  A,H
       CMP  D
       JZ   DS4       ;YES, DONE, WRITE CTRL-Z
DS2    MOV  A,M       ;FETCH PROGRAM BYTE
       INX  H         ;ADVANCE POINTER
       CPI  ' '       ;CONTROL CHAR?
       JM   DS3       ;YES, HANDLE AS OCT
       OUT  5         ;NO, WRITE IT
       CPI  '\'       ;DOUBLE THE ESC CHAR
       JNZ  DS1
       OUT  5
       JMP  DS1
DS3    MOV  B,A       ;WRITE "\OOO"
       MVI  A,'\'     ;WRITE ESC CHAR
       OUT  5
       MOV  A,B       ;HIGHEST OCT DIGIT
       RLC
       RLC
       ANI  3         ;JUST TWO VALID BITS
       ADI  '0'
       OUT  5
       MOV  A,B       ;MIDDLE OCT DIGIT
       RRC
       RRC
       RRC
       ANI  7
       ADI  '0'
       OUT  5
       MOV  A,B       ;LOWEST OCT DIGIT
       ANI  7
       ADI  '0'
       OUT  5
       JMP  DS1
DS4    MVI  A,1AH     ;WRITE END MARKER CTRL-Z
       OUT  5
       RST  0
;*
DLOAD  CALL ENDCHK    ;*** LOAD(CR) ***
       LXI  H,TXTBGN  ;POINTER TO FIRST PROG LOCATION
DL1    CALL TAPIN     ;END REACHED CTRL-Z?
       JZ   DL9       ;EOF, STORE POINTER
       CPI  '\'       ;NO, CHECK ESC
       JZ   DL3       ;YES, HANDLE ESC
DL2    MOV  M,A       ;NO, STORE CHARACTER
       INX  H
       JMP  DL1
DL3    LXI  B,3       ;LOOP THREE DIGITS
DL4    MOV  A,B       ;MAKE ROOM FOR NEXT DIGIT IN B
       RLC
       RLC
       RLC
       MOV  B,A
       CALL TAPIN     ;READ NEXT CHAR
       JZ   DL9       ;EOF, STORE POINTER
       CPI  '0'       ;OCT DIGIT?
       JM   DL2       ;NO, JUST WRITE IT
       CPI  '8'
       JP   DL2
       SUI  '0'       ;CONVERT OCT TO BIN
       ORA  B         ;PATCH TOGETHER
       MOV  B,A       ;STORE BACK TO B
       DCR  C         ;LOOP END REACHED
       JNZ  DL4       ;NO, NEXT DIGIT
       JMP  DL2       ;YES, WRITE BYTE INTO MEM
DL9    SHLD TXTUNF    ;STORE FIRST UNUSED LOCATION
       RST  0
;*
TAPIN  IN   4         ;TAPE READY?
       ORA  A
       JZ   TAPIN     ;NO, POLL
       IN   5         ;YES, READ
       CPI  1AH       ;Z-FLAG FOR EOF
       RET
;*
;*************************************************************
;*
...
OC3    POP  PSW       ;GET OLD A BACK
       OUT  1         ;WRITE TO CONSOLE
       CPI  0DH       ;WAS IT A 'CR'?
       RNZ            ;NO, DONE
       PUSH PSW
       MVI  A,0AH     ;GET LINEFEDD
       OUT  1         ;WRITE TO CONSOLE
       POP  PSW
       RET
CHKIO  IN   0         ;READ CONSOLE STATUS (0FFH==avail)
       ORA  A         ;SET FLAGS
       RZ             ;NOTHING, RETURN
       IN   1         ;READ CONSOLE CHAR
       ANI  7FH       ;CLEAR BIT 8
...
CI1    CPI  3         ;IS IT CONTROL-C?
       JZ   RSTART    ;YES, RESTART TBI
       CPI  8         ;IS IT BS?
       JNZ  CI2       ;NO, ECHO IT
       MVI  A,177Q    ;MAKE IT DEL
       ORA  A         ;SET NZ
       RET
CI2    CPI  0DH       ;CR?
       JZ   CI3       ;YES, ECHO, DONE
       CPI  ' '       ;CONTROL CHAR?
       JP   CI3       ;NO, ECHO IR
       XRI  A         ;YES, IGNORE IT, SET Z
       RET
CI3    OUT  1         ;ECHO CHAR ON CONSOLE
       ORA  A         ;SET NZ
       RET
;*
;**************************************************************
;*
;* QUIT THE PROGRAM BY HALTING THE PROCESSOR
;*
BYE    HLT
       RST  0         ;RESTART IF CONTINUED
;*
...
RANPNT DW   RSTART    ;RANDOM NUMBER POINTER
TXTUNF DW   TXTBGN    ;->UNFILLED TEXT AREA
TXTBGN DS   1         ;TEXT SAVE AREA BEGINS
;*
;* WILL BE OVERWRITTEN BY USER PROGRAM
;*
MSG1   DB   0CH,'LI-CHEN WANG TINY BASIC VER. 1.0 10-JUN-1976',0DH,0
INIT   MVI  A,0FFH
       STA  OCSW      ;TURN ON OUTPUT SWITCH
       SUB  A         ;CLEAR ACCUMULATOR
       LXI  D,MSG1    ;GET INIT MESSAGE
       CALL PRTSTG    ;SEND IT
       LXI  H,ST1     ;PATCH RST 0 CODE
       SHLD INIPAT+1
       RST  0
;*
       ORG  1F00H
